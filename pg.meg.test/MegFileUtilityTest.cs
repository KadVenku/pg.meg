...
namespace pg.meg.test
{
    ...
    public class MegFileUtilityTest
    {
        private static readonly string TEST_DATA_PATH_IN =
            Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "test_data",
                "eaw_patch_2.meg"));

        private static readonly string TEST_DATA_PATH_BUILD = Path.GetFullPath(
            Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "test_data", "eaw_patch_build.meg"));
        ...
        public void UnpackMegFile_TestSuccess()
        {
            ...
            foreach (string file in Directory.GetFiles(TEST_DATA_PATH_OUT))
            {
